...
class BuildRunnerDaemonBuilder implements DaemonBuilder {
  ...
  Future<void> build(
    ...
    var outputMap = <String, List<String>>{};
    for (var target in defaultTargets) {
      outputMap[target.target] ??= [];
      if (target.output?.isNotEmpty ?? false) {
        outputMap[target.target].add(target.output);
      }
    }
    try {
      var mergedChanges = collectChanges([changes]);
      var result = await _builder.run(mergedChanges, outputMap: outputMap);
